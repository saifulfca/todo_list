...
- **Critical path & slack** ⚡ — proper CPM scheduling. Each project shows its **slack/float** (how many days it can slip before it delays the plan) or an **⚡ Critical** badge when it has zero slack.
- **Critical Path planner (pop-up)** — click **⚡ Critical Path** (in the Calendar header, or the chip that appears in any notebook with projects) to open a focused **mini-Gantt**: one project per row on a single timeline, showing the whole **A→B→C chain with arrows**, the critical path in red, and each project's slack. **Scope it to a single notebook or all** with the dropdown. Right inside the pop-up you can **drag a bar to reschedule, drag its edges to resize, drag the ● dot onto another project to link them, and click ✕ on an arrow to unlink** — dependent projects cascade automatically.
